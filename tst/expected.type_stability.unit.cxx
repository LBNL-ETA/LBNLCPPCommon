...
// Mapper #2: int -> double (plain value)
...
TEST(ExpectedExt_AndThen, PlainValueFunc_WrappedIntoExpected)
{
    ...
    auto r1 = err.and_then(to_double_plain);
...
TEST(ExpectedExt_Map, MapsToDifferentT)
{
    ...
    auto r2 = err.map([](int x) { return x * 2.0; });
    EXPECT_FALSE(r2.has_value());
    EXPECT_EQ(r2.error(), "x");
}

TEST(ExpectedExt_MapError, MapsError_PreservesValue)
{
    const auto err = lbnl::make_unexpected<int, std::string>(std::string{"oops"});
    auto r = err.map_error([](const std::string & e) { return e.size(); });   // E -> size_t
    ...
    auto r2 = ok.map_error([](const std::string & e) { return e.size(); });
...
TEST(ExpectedExt_Map, MapsValue_PreservesErrorType)
{
    const auto ok = lbnl::make_expected<int, std::string>(3);
    auto r = ok.map([](int x) { return x * 2.0; });   // int -> double
...
TEST(ExpectedExt_AndThen, ValueFunc_WrappedIntoExpected)
{
    const auto ok = lbnl::make_expected<int, std::string>(4);
    auto r = ok.and_then([](int x) { return x * 1.5; });   // int -> double (plain value)
    static_assert(std::is_same_v<decltype(r), lbnl::ExpectedExt<double, std::string>>);
    ASSERT_TRUE(r.has_value());
    EXPECT_DOUBLE_EQ(r.value(), 6.0);
}

TEST(ExpectedExt_MapError, MapsErrorType)
{
    const auto err = lbnl::make_unexpected<int, std::string>("oops");
    auto r = err.map_error([](const std::string & e) { return e.size(); });   // E: string -> size_t
    static_assert(std::is_same_v<decltype(r), lbnl::ExpectedExt<int, std::size_t>>);
    EXPECT_FALSE(r.has_value());
    EXPECT_EQ(r.error(), std::string("oops").size());
}
